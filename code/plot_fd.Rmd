...
all_sweeps<- full_join(sweeps_caudatus.df, sweeps_cruentus.df) %>%
  full_join(sweeps_hypochondriacus.df)

sweeps_cau_cru<- full_join(sweeps_caudatus.df, sweeps_cruentus.df) 

  sweeps_cau_hyp<- full_join(sweeps_caudatus.df, sweeps_hypochondriacus.df) 

interesection_crops_tree_sweep_tops<- bedtoolsr::bt.intersect(top_windows_crops_tree, sweeps_cau_hyp) %>%
  rename(chr=V1, windowStart=V2, windowEnd=V3, f_dM=V6) %>%
          mutate(window_center=round(((windowStart+windowEnd)/2))) %>%
              mutate(Position = window_center/1e6) 
  

interesection_crops_tree_sweep_bottom<- bedtoolsr::bt.intersect(bottom_windows_crops_tree, sweeps_cau_cru) %>%
  rename(chr=V1, windowStart=V2, windowEnd=V3, f_dM=V6) %>%
          mutate(window_center=round(((windowStart+windowEnd)/2))) %>%
              mutate(Position = window_center/1e6) 
