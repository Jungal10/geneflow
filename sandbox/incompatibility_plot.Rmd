...
dados_30d<- fread("/Users/josedias/Downloads/incompatibility_study - documentation_post_transp_updat (1).tsv" )

dados_30d %>%
  group_by(cross_type) %>%
  dplyr::summarise(plyr::count(died)) %>%
...
dados_30d %>%
    # group_by(cross_type) %>%
ggplot(., aes(x=as.factor(cross_type), y=as.numeric(`growth_93d`)), fill=cross_type) +
geom_beeswarm(aes(color=type)) +
nice_layout

dados_30d %>%
    melt(id.vars="cross_type") %>%
  filter(grep("growth"), variable)
