...
for (i in 1:24) { #cicles per file == number of pop
      # ... make some data
...
# especies<- c( "caudatus", "cruentus", "hypochondriacus", "hybridus_CA", "hybridus_SA", "quitensis")
especies<-"cruentus"
subsample_chr_list<-(c(16:16)) #ubsample due to exhausted vector
...
# 
# scaffold_8_lista %>%
# fwrite(., "../data/processed/AS_files/AS_list_sc16.txt", quote=FALSE, sep="\t")
# 
# lista %>%
#   filter(recipient=="cruentus")
# 
# sub_scaffold_16_lista  <-
#   scaffold_16_lista %>%
#   head(n=300)
# 

cruentus_summary_sc16 <-
cruentus_lista_sc16  %>%
      # filter(pos <40000) %>%
  rowwise() %>%
  dplyr::mutate(count_caudatus = (sum(c_across(cruentus1:cruentus9) == "caudatus", na.rm=TRUE))/21,
         count_cruentus = (sum(c_across(cruentus1:cruentus9) == "cruentus", na.rm=TRUE))/21,
          count_hypochondriacus = (sum(c_across(cruentus1:cruentus9) == "hypochondriacus", na.rm=TRUE))/21,
           count_hybridus_CA = (sum(c_across(cruentus1:cruentus9) == "hybridus_CA", na.rm=TRUE))/21,
           count_hybridus_SA = (sum(c_across(cruentus1:cruentus9) == "hybridus_SA", na.rm=TRUE))/21,
          count_quintensis = (sum(c_across(cruentus1:cruentus9) == "quitensis", na.rm=TRUE))/21,
                   count_ambiguous = (sum(c_across(cruentus1:cruentus9) == "ambiguous", na.rm=TRUE))/21
         ) %>%
  ungroup() %>%
        select(pos, chr, count_caudatus,count_cruentus,count_hypochondriacus,count_quintensis,count_hybridus_CA, count_hybridus_SA, count_ambiguous ) 

# 
# quitensis_summary_sc9 <-
# quitensis_lista_sc9  %>%
#       # filter(pos <40000) %>%
#   rowwise() %>%
#   dplyr::mutate(count_caudatus = (sum(c_across(quitensis1:quitensis17) == "caudatus", na.rm=TRUE))/24,
#          count_cruentus = (sum(c_across(quitensis1:quitensis17) == "cruentus", na.rm=TRUE))/24,
#           count_hypochondriacus = (sum(c_across(quitensis1:quitensis17) == "hypochondriacus", na.rm=TRUE))/24,
#            count_hybridus_CA = (sum(c_across(quitensis1:quitensis17) == "hybridus_CA", na.rm=TRUE))/24,
#            count_hybridus_SA = (sum(c_across(quitensis1:quitensis17) == "hybridus_SA", na.rm=TRUE))/24,
#           count_quintensis = (sum(c_across(quitensis1:quitensis17) == "quitensis", na.rm=TRUE))/24,
#                    count_ambiguous = (sum(c_across(quitensis1:quitensis17) == "ambiguous", na.rm=TRUE))/24
#          ) %>%
#   ungroup() %>%
#         select(pos, chr, count_caudatus,count_cruentus,count_hypochondriacus,count_quintensis,count_hybridus_CA, count_hybridus_SA, count_ambiguous ) 


fwrite(cruentus_summary_sc16, "/Users/josedias/Desktop/cruentus_summary_sc16.txt", sep="\t", quote=F)
...
count_stackbarcolors<- c("count_ambiguous"="#808080", "count_caudatus"="#a6cee3", "count_cruentus"="#1f78b4", "count_hypochondriacus"="#b2df8a", "count_hybridus_CA"="#33a02c", "count_hybridus_SA"="#f8bf68", "count_quintensis"="#fb9a99")

# quitensis_lista_sc9  %>%
#       filter(pos <40000) %>%
#   rowwise() %>%
#   dplyr::mutate(count_caudatus = sum(c_across(cruentus1:cruentus9) == "caudatus", na.rm=TRUE),
#          count_cruentus = sum(c_across(cruentus1:cruentus9) == "cruentus", na.rm=TRUE),
#           count_hypochondriacus = sum(c_across(cruentus1:cruentus9) == "hypochondriacus", na.rm=TRUE),
#            count_hybridus_CA = sum(c_across(cruentus1:cruentus9) == "hybridus_CA", na.rm=TRUE),
#            count_hybridus_SA = sum(c_across(cruentus1:cruentus9) == "hybridus_SA", na.rm=TRUE),
#           count_quintensis = sum(c_across(cruentus1:cruentus9) == "quitensis", na.rm=TRUE),
#                    count_ambiguous = sum(c_across(cruentus1:cruentus9) == "ambiguous", na.rm=TRUE)
#          ) %>%
#   ungroup() %>%
#         select(pos, chr, count_caudatus,count_cruentus,count_hypochondriacus,count_quintensis,count_hybridus_CA, count_hybridus_SA, count_ambiguous ) 
...
plot_sc16_cruentus<- cruentus_summary_sc16 %>%
    # filter(pos <40000) %>%
         # group_by(pos) %>%
    melt(, id.vars=c("pos", "chr"))   %>%
  # filter(pos=="13284") 
              ggplot(., aes(x = as.factor(pos), y = value, fill = variable)) + 
     # ggtitle(paste0(species, " recipient scaffold_9")) +
        geom_bar( stat='identity',  width=1) +
  scale_fill_manual(values=count_stackbarcolors) +
    scale_color_manual(values=count_stackbarcolors) +
    # scale_x_discrete(name="position", expand = waiver(), guide = waiver()) +
      scale_y_continuous(expand = c(0, 0)) +
  ylim(0,1) + # expand=c(0,0)removes space between plot area and x axis  +
...
# 
# ggplot() +
#   geom_point(aes(x=pos, y=count_caudatus)) +
#     geom_point(aes(x=pos, y=count_cruentus)) +
#     geom_point(aes(x=pos, y=count_hypochondriacus)) +
#     geom_point(aes(x=pos, y=count_hybridus_CA)) +
#     geom_point(aes(x=pos, y=count_hybridus_SA)) +
#     geom_point(aes(x=pos, y=count_quintensis)) +
#   scale_fill_manual(values=count_stackbarcolors) +
#     scale_color_manual(values=count_stackbarcolors) 

 ggsave(filename="/Users/josedias/Desktop/summary_cruentus_sc16.png", plot_sc16_cruentus, width = 14, height=3, bg="white")

```


```{r}
plot_smooth_quitensis<-
  quitensis_summary_sc9 %>%
    # filter(pos <40000) %>%
  # filter(chr==13) %>%
  # filter(pos<100000) %>%
  ggplot() +
  # geom_line() +z`z`
  geom_smooth( aes(x = as.numeric(pos), y=count_quintensis, color="quitensis")) +
        geom_smooth( aes(x = as.numeric(pos), y=count_ambiguous, color="ambiguous")) +
    # geom_point(aes(x = pos, y=caudatus, color="caudatus"), size=1) +
    geom_smooth(aes(x = pos, y=count_cruentus, color="cruentus")) +
    # geom_point(aes(x = pos, y=cruentus, color="cruentus"), size=1) +
      geom_smooth(aes(x = pos, y=count_caudatus, color="caudatus")) +
    # geom_point(aes(x = pos, y=hypochondriacus, color="hypochondriacus"), size=1) +
        geom_smooth(aes(x = pos, y=count_hybridus_CA, color="hybridus_CA")) +
    # geom_point(aes(x = pos, y=hybridus_CA, color="hybridus_CA"), size=1) +
        geom_smooth(aes(x = pos, y=count_hybridus_SA, color="hybridus_SA")) +
 ...
 ggsave(filename="/Users/josedias/Desktop/smooth_summary_quitensis_sc9.png", plot_smooth_quitensis, width = 14, height=3, bg="white")
...
species<-"hybridus_SA"
column_names<-c("pos", "caudatus", "cruentus", "hypochondriacus", "hybridus_CA", "hybridus_SA", "quitensis")
subsample_chr_list<-(c(1:9)) #ubsample due to exhausted vector
lista=list()
sumario=list()
...
ggsave("/Users/josedias/Desktop/sumarios/summary_donors_boxplot_narrow.png",
ggplot() +
  geom_boxplot(data = whole_sumario, aes(x=recipient, y=value, fill=donor, color=donor)) +
   # geom_beeswarm(data = whole_sumario, aes(x=recipient, y=value, color=donor, cex=0.5)) +
...
,
height =5, width =20, bg="white" )



ggsave("/Users/josedias/Documents/PhD/projects/GeneFlow/text/amaranth_introgression/figures/summary_donors_quitensis_nohybridus.png",
ggplot() +
  geom_boxplot(data = sumario_qui_notself, aes(x=recipient, y=value, fill=donor, color=donor)) +
   # geom_beeswarm(data = whole_sumario, aes(x=recipient, y=value, color=donor, cex=0.5)) +
  # geom_quasirandom(method = "pseudorandom") +
  # facet_wrap(recipient~.) +
    # facet_grid(.~recipient, scales = "free") +
...
# ca.sa.countries <- c('Argentina', 'Bolivia', 'Brazil',  
#                'Colombia', 'Chile','Ecuador','French Guiana','Guyana', 
#                'Paraguay', 'Peru', 'Suriname', 
#                'Trinidad and Tobago', 'Uruguay', 'Venezuela', 'Mexico', 'Guatemala')
# 
# countries.maps <- map_data("world", region = ca.sa.countries)
# 
# 
# us <- c(left = -122.19, bottom = -59.87, right = -25.28, top = 32.72)
# get_stamenmap(us, zoom = 5, maptype = "toner-lite") %>% ggmap(legend = "none") 
...
sumario_cau_notself<- sumario_cau %>%
  filter(donor !="caudatus") %>%
    filter(donor !="hybridus_CA") %>%
    filter(donor !="hybridus_SA") 


sumario_cru_notself<- sumario_cru %>%
  filter(donor !="cruentus") %>%
    filter(donor !="hybridus_CA") %>%
    filter(donor !="hybridus_SA")

sumario_hyp_notself<- sumario_hyp %>%
  filter(donor !="hypochondriacus") %>%
    filter(donor !="hybridus_CA") %>%
    filter(donor !="hybridus_SA")

sumario_hybCA_notself<- sumario_hybca %>%
  filter(donor !="hybridus_CA") %>%
    filter(donor !="hybridus_CA") %>%
    filter(donor !="hybridus_SA")

sumario_hybSA_notself<- sumario_hybsa %>%
  filter(donor !="hybridus_SA") %>%
    filter(donor !="hybridus_CA") %>%
    filter(donor !="hybridus_SA")

sumario_qui_notself<- sumario_qui %>%
  filter(donor !="quitensis") %>%
    filter(donor !="hybridus_CA") %>%
    filter(donor !="hybridus_SA")


  whole_sumario_notself<- rbind(sumario_cau_notself,sumario_cru_notself,sumario_hyp_notself,sumario_hybCA_notself,sumario_hybSA_notself ,sumario_qui_notself)


ggsave("/Users/josedias/Desktop/sumarios/summary_donors_notself_boxplot_narrow_w_legend.png",

ggplot() +
  geom_boxplot(data = whole_sumario_notself, aes(x=recipient, y=value, fill=donor, color=donor)) +
   # geom_beeswarm(data = whole_sumario, aes(x=recipient, y=value, color=donor, cex=0.5)) +
  # geom_quasirandom(method = "pseudorandom") +
  # facet_wrap(recipient~.) +
    facet_grid(.~recipient, scales = "free") +
...
# ggsave("../data/processed/figures/hypochondriacus_21_proportionsummary_chromosome_sc9.png",
       ggsave("/Users/josedias/Desktop/cruentus_5_proportionsummary_chromosome_sc16.png",
...
# summarise(n_distinct(variable))
  ...
  geom_bar(stat='identity', width = 1, na.rm = T) +
          # geom_bar( stat='identity',  width=1)  +
    # ylim(0,1) +
    ggtitle("hypochondriacus 643058 sc16") +
  ylab("proportion") +
      nice_layout +
          scale_color_manual(values = stackbarcolors) +
            scale_fill_manual(values = stackbarcolors) +
...
# height = 4, width =15)
  ...
  ggsave("/Users/josedias/Desktop/cruentus_5_smooth_chromosome_sc16.png",
...
for (species in especies){
  nam <- paste( species,  "plot_per_site_sc16", sep = "_")
assign(nam, lista %>% 
...
plot_to_save<- hybridus_SA_plot_per_site_sc16 %>%
         # filter(pos<=14261388)  %>%
         # filter(pos>= 14201388) %>%
...
line_plot_to_save<- hybridus_SA_plot_per_site_sc16 %>%
...
line_plot_to_save_QTL3<- hybridus_SA_plot_per_site_sc16 %>%
  filter(pos<5850000) %>%
  filter(pos>6000000) %>%
    mutate(Position_kb = pos/1e3) %>%
...
#  
 ...
 ggsave(filename="/Users/josedias/mount/projects/ag-stetter/jdias/projects/geneflow/data/processed/figures/hybridus_SA_line_plot_per_site_sc16.png", line_plot_to_save, width = 14, height=3)
 

 ggsave(filename="/Users/josedias/mount/projects/ag-stetter/jdias/projects/geneflow/data/processed/figures/hybridus_SA_line_plot_per_site_QTL_highlighted_sc16.png", line_plot_to_save_QTL3, width = 14, height=3)
